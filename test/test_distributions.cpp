...
BOOST_AUTO_TEST_CASE(ln_normal_den_test) {
    auto X = cvec<double>({0.5, 1, -0.5}), mu = psca(1.0), s = psca(2.0);

    auto y1 = ln_normal_den(X, mu, s);
    GraphManager gm1(y1);
    gm1.run();

    BOOST_CHECK_CLOSE(y1->v(), -5.148757, 1e-2);
    BOOST_CHECK_CLOSE(mu->g(), -0.5, 1e-5);
    BOOST_CHECK_CLOSE(s->g(), -1.1875, 1e-5);
}
BOOST_AUTO_TEST_CASE(ln_t_den_test) {
    auto X = cvec<double>({0.5, 1, -0.5}), mu = psca(1.0), s = psca(2.0), nu = psca(2.0);

    auto y1 = ln_t_den(X, mu, s, nu);
    GraphManager gm1(y1);
    gm1.run();

    BOOST_CHECK_CLOSE(y1->v(), -5.6165156, 1e-5);
    BOOST_CHECK_CLOSE(mu->g(), -0.620842572, 1e-5);
    BOOST_CHECK_CLOSE(s->g(), -1.1252771618, 1e-5);
    BOOST_CHECK_CLOSE(nu->g(), 0.218615966, 1e-5);
}
BOOST_AUTO_TEST_CASE(mixture_test1) {
    // auto X = load_mat("gmm_datat5000_10.txt");
    auto X = std::make_shared<Var<>>("gmm_datat5000_10.txt");
    assert(X->cols() == 10);
    auto I = cmat(10, 10);
    *I = TMat<>::Identity(10, 10);
    ...
    auto p = sigmoid(w1);
    auto p1v = exp(-0.5 * diag(Xmu1 * inv(S1), transpose(Xmu1)));
    auto p2v = exp(-0.5 * diag(Xmu2 * inv(S2), transpose(Xmu2)));
    // auto ll = -sum(ln(p * p1v + (1 - p) * p2v));
    auto ll = -sum(ln(p * p1v));
    GraphManager<> gm(ll);
    cout << "w1: " << w1->val() << ", grad before: " << w1->grad();
    // for(int i=0;i<X.rows();++i){
    gm.run();
    BOOST_CHECK_CLOSE(ll->v(), 477800.817550866, 1e-5);
    cout << ", grad after " << w1->grad() << endl;
    cout << "mu1 grad: " << mu1->grad() << endl;
    cout << "mu2 grad: " << mu2->grad() << endl;
	cout << "sigma1 grad: " << sigma1->grad() << endl;
}
BOOST_AUTO_TEST_CASE(mixture_test) {
    ...
    *I = TMat<>::Identity(10, 10);
    ...
    // for(int i=0;i<X.rows();++i){
    gm.run();
    BOOST_CHECK_CLOSE(ll->v(), 378180.344216452, 1e-5);
    BOOST_CHECK_CLOSE(w1->g(), 95.9572941064835, 1e-5);
	BOOST_CHECK_CLOSE(sigma1->g(), -287.741417996585, 1e-5);
    cout << ", grad after " << w1->grad() << endl;
    cout << "mu1 grad: " << mu1->grad() << endl;
    cout << "mu2 grad: " << mu2->grad() << endl;
	cout<<"sigma1 grad: "<<sigma1->grad()<<endl;
