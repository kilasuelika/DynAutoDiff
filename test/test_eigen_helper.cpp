...
BOOST_AUTO_TEST_CASE(load_mat_test) {
    ...
    BOOST_CHECK_CLOSE(X.coeff(0, 0), 0.814723686393179, TL);
    BOOST_CHECK_CLOSE(X.coeff(5, 4), 0.699887849928292, TL);
    BOOST_CHECK_CLOSE(X.coeff(99, 4), 0.613460736812875, TL);

    auto y = load_mat("y.txt");

    BOOST_CHECK_EQUAL(y.rows(), 100);
    BOOST_CHECK_EQUAL(y.cols(), 1);
    BOOST_CHECK_CLOSE(y.coeff(99, 0), 7.62241903286546, TL);
}
